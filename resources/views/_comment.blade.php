...
@php($isCommentDeleted = !is_null($comment->deleted_at))
            ...
            <li id="comment-{{ $comment->id }}" class="media">
                ...
                <div class="media-body">
                    <h5 class="mt-0 mb-1">{{ $comment->commenter->name ?? $comment->guest_name }}
                        @if($isCommentDeleted)
                            <small class="text-muted"> - Deleted {{ $comment->deleted_at->diffForHumans() }} </small>
                        @else
                    ...
                    <div>
                        @if ( !isset($ticket) || $ticket->status->name != 'Closed' )
